	...
	public function __construct($data)
	{
		...
		if(strlen($data) == 61) {
			...
			$this->date = sprintf("20%02d-%02d-%02d %02d:%02d:%02d" 
								, $package["years"], $package["months"], $package["days"]
								, $package["hours"], $package["minutes"], $package["seconds"]);
		}
		else {
...
//          echo $this->date."\n";
		...
		$package = unpack("v16analog/vdigital/C4speed/Cactive/Vpower1/vkWh1/vMWh1/Vpower2/vkWh2/vMWh2",$data);
		// 16 Analog channels
		for($i=1; $i<=16; $i++) {
			$key = ("analog".$i);
		...
		for($i=1; $i<=2; $i++) {
			$key = ("energy".$i);
			$this->$key = self::convertEnergy($package["MWh".$i], $package["kWh".$i], $package["active"], $i);
		}
		// 2 power values
		for($i=1; $i<=2; $i++) {
			$key = ("power".$i);
	...
	*/
	...
	{	
		if (self::DEBUG > 1) echo "convertAnalog: ".$value."\n";
		// choose type
	...
	*/
	...
	{
		if (self::DEBUG > 1) echo "convertDigital: ".$value."\n";
	...
	*/
	...
	{
		if (self::DEBUG > 1) echo "convertSpeed: ".$value."\n";
	...
	*/
	...
	{
		if (self::DEBUG > 1) echo "convertEnergy: ".$kWh."\n";
		if($active & $position) {
	...
	*/
	...
	{
		if (self::DEBUG > 1) echo "convertPower: ".$value."\n";
		if($active & $position) {
	...
	*/
	...
	{
		if (self::DEBUG > 1) echo "calculateValue: ".$valu."\n";		
		$result = $value & $positivemask;
		if($value & $signbit) {
			$result = -(($result ^ $positivemask) + 1);
		}
		return $result * $multiplier;
		//return number_format('$result * $multiplier', 2);
	}
	/**
	...
	*/
	...
	{
		if (self::DEBUG > 1) echo "convertRASMode - RASMODE value = ".$value."\n"; 
		// choose type
		switch($value & self::TYPE_MASK)
		{
			case self::TYPE_RAS:
				if (self::DEBUG > 1) echo "convertRASMode - TYPE_RAS\n"; 
			...
			default:
				if (self::DEBUG > 1 ) echo "notUsed = ".$config->notUsed."\n"; 
